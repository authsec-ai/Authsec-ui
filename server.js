...
app.get("/health", (req, res) => {
  ...
  console.log(
    `[${healthStatus.timestamp}] Health check - VITE_API_URL: ${healthStatus.environment.VITE_API_URL}, Amplitude: ${healthStatus.environment.VITE_AMPLITUDE_API_KEY}, Clarity: ${healthStatus.environment.VITE_CLARITY_PROJECT_ID}, HubSpot: ${healthStatus.environment.VITE_HUBSPOT_ACCESS_TOKEN}`,
  );
...
app.listen(port, () => {
  ...
  console.log(
    `[${new Date().toISOString()}] VITE_API_URL: ${process.env.VITE_API_URL || "not set"}`,
  );
});
