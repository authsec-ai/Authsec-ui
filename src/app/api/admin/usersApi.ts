...
export const adminUsersApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAdminUsers: builder.query<AdminUsersResponse, AdminUsersQueryParams>({
      query: (params = {}) => {
        console.log(
          "[ADMIN API] 🟡 getAdminUsers called - THIS SHOULD BE CALLED FROM ADMIN USERS PAGE",
          {
            page: params.page,
            limit: params.limit,
        ...
        if (params.is_synced_user !== undefined)
          body.is_synced_user = params.is_synced_user;
    ...
    setAdminUserActive: builder.mutation<
      any,
