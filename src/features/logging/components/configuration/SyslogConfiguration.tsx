...
interface SyslogConfigurationProps {
  config: {
    workspace_id: string;
    host: string;
    port: string;
...
export function SyslogConfiguration({ config, onChange, onSave, onCancel, isLoading }: SyslogConfigurationProps) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border border-slate-200/50 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/70 backdrop-blur-sm shadow-lg dark:shadow-[0_18px_32px_rgba(8,8,12,0.38)]">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg text-slate-900 dark:text-zinc-100">Syslog Configuration</CardTitle>
                <CardDescription className="text-sm text-slate-600 dark:text-zinc-500">
                  Stream logs to your Syslog server
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div>
              <Label htmlFor="syslog-tenant-id" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                Tenant ID *
              </Label>
              <Input
                id="syslog-tenant-id"
            ...
            {/* Host (domain or IP) */}
            <div>
              <Label htmlFor="syslog-host" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                Host *
              </Label>
              <Input
                id="syslog-host"
            ...
            {showPort && (
              <div>
                <Label htmlFor="syslog-port" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                  Port *
                </Label>
                <Input
                  id="syslog-port"
      ...
      <div>
        <Card className="border border-slate-200/50 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/70 backdrop-blur-sm shadow-lg dark:shadow-[0_16px_28px_rgba(8,8,12,0.32)]">
          <CardHeader>
            <CardTitle className="text-lg text-slate-900 dark:text-zinc-100">About Syslog</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-zinc-500 leading-relaxed">
              Stream logs to your Syslog server following RFC5424 conventions.
            </p>
