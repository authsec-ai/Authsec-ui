...
export function SplunkConfiguration({ config, onChange, onSave, onCancel, isLoading }: SplunkConfigurationProps) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border border-slate-200/50 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/70 backdrop-blur-sm shadow-lg dark:shadow-[0_18px_32px_rgba(8,8,12,0.38)]">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg text-slate-900 dark:text-zinc-100">Splunk Integration</CardTitle>
                <CardDescription className="text-sm text-slate-600 dark:text-zinc-500">
                  Connect to your Splunk instance via HTTP Event Collector
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div>
              <Label htmlFor="splunk-tenant-id" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                Tenant ID *
              </Label>
              <Input
                id="splunk-tenant-id"
            ...
            {/* Host (domain or IP) */}
            <div>
              <Label htmlFor="splunk-host" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                Host *
              </Label>
              <Input
                id="splunk-host"
            ...
            {showPort && (
              <div>
                <Label htmlFor="splunk-port" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                  Port *
                </Label>
                <Input
                  id="splunk-port"
      ...
      <div>
        <Card className="border border-slate-200/50 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/70 backdrop-blur-sm shadow-lg dark:shadow-[0_16px_28px_rgba(8,8,12,0.32)]">
          <CardHeader>
            <CardTitle className="text-lg text-slate-900 dark:text-zinc-100">About Splunk</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-zinc-500 leading-relaxed">
              Splunk allows you to search, analyze, and visualize machine data in real-time,
