...
export function ElasticsearchConfiguration({ config, onChange, onSave, onCancel, isLoading }: ElasticsearchConfigurationProps) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border border-slate-200/50 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/70 backdrop-blur-sm shadow-lg dark:shadow-[0_18px_32px_rgba(8,8,12,0.38)]">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg text-slate-900 dark:text-zinc-100">Elasticsearch Configuration</CardTitle>
                <CardDescription className="text-sm text-slate-600 dark:text-zinc-500">
                  Index logs directly into Elasticsearch
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div>
              <Label htmlFor="es-tenant-id" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                Tenant ID *
              </Label>
              <Input
                id="es-tenant-id"
            ...
            {/* Host (domain or IP) */}
            <div>
              <Label htmlFor="es-host" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                Host *
              </Label>
              <Input
                id="es-host"
            ...
            {showPort && (
              <div>
                <Label htmlFor="es-port" className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-zinc-500">
                  Port *
                </Label>
                <Input
                  id="es-port"
      ...
      <div>
        <Card className="border border-slate-200/50 dark:border-neutral-800/80 bg-white/80 dark:bg-neutral-950/70 backdrop-blur-sm shadow-lg dark:shadow-[0_16px_28px_rgba(8,8,12,0.32)]">
          <CardHeader>
            <CardTitle className="text-lg text-slate-900 dark:text-zinc-100">About Elasticsearch</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-zinc-500 leading-relaxed">
              Index your logs into Elasticsearch for powerful search and analysis capabilities.
            </p>
