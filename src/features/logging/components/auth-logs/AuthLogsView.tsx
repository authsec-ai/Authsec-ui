...
export function AuthLogsView({
  ...
  type LevelStyle = { text: string; icon: string };

  const statusStyles: Record<string, LevelStyle> = {
    success: {
  ...
  const getStatusStyle = (status: AuthLog["status"]): LevelStyle =>
    statusStyles[status] ?? statusStyles.failure;
  ...
  /**
   * Format a log entry into console-style text lines.
   * Only real backend fields are rendered — no fabricated data.
   */
  const formatLogEntry = (log: AuthLog) => {
    const timestamp = new Date(log.timestamp).toISOString();
    const status = log.status.toUpperCase().padEnd(10);

    // Main line: timestamp + status + action + actor info
    const actor = log.userId || "—";
    const realm = log.actorRealm ? `[${log.actorRealm}]` : "";
    const mainLine = `[${timestamp}] ${status} ${realm} user=${actor} action=${log.action ?? "—"}`;

    // Details line: real fields only
    const details: string[] = [];
    if (log.ipAddress) details.push(`ip=${log.ipAddress}`);
    if (log.requestId) details.push(`request_id=${log.requestId}`);

    // Failure reason
    const reasonLine = log.failureReason
                ...
                {logs.map((log) => {
                  ...
                  return (
                    <div key={log.id} className="py-3">
                      ...
                      {/* Expanded detail panel — only real fields */}
                      {isExpanded && (
                          ...
                          <div className="space-y-3 text-[12px] font-mono">
                            ...
                            <div>
                              ...
                              <div className="pl-3 space-y-1 text-slate-600 dark:text-zinc-400">
                                ...
                                {log.actorRealm && <div>• Actor Realm: {log.actorRealm}</div>}
                              </div>
                            </div>
                            ...
                            {log.failureReason && (
                              <div>
                                ...
                                <div className="pl-3 space-y-1 text-slate-600 dark:text-zinc-400">
                                  <div>• Reason: {log.failureReason}</div>
                                </div>
                              </div>
                            )}
                          </div>
