...
import { FileSearch, Settings, AlertCircle } from "lucide-react";
import {
  AuditLogsView,
...
export function AuditLogsPage() {
  ...
  const [filters, setFilters] = useState<AuditLogsFilterParams>({});
  ...
  // Filter logs based on filter params (only client-side filters)
  // Note: action/operation, start_time, end_time, sort_by, sort_desc are handled server-side
  const filteredLogs = useMemo(() => {
    return auditLogs.filter((log) => {
      ...
      const matchesRollback =
        !filters.showRollbackOnly || log.rollbackAvailable === true;
  ...
  const handleFiltersChange = (newFilters: AuditLogsFilterParams) => {
