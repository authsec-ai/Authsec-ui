...
interface RoleScopeMappingsViewProps {
  searchQuery: string;
...
export function RoleScopeMappingsView({
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          const displayName = hasUsername
            ...
            : (row.original.user_id ? `User ${row.original.user_id.substring(0, 8)}...` : "-");

          return (
            <div className="space-y-0.5">
              <div
                className={`font-medium truncate ${hasUsername ? 'text-foreground' : 'text-foreground/60'}`}
                title={row.original.username || row.original.user_id}
              >
                {displayName}
              </div>
              {row.original.email && (
  ...
  return (
    <ResponsiveTableProvider tableType="roleScopeMappings">
