pipeline {
    ...
    // /opt/authsec/src/Authsec-ui, builds the image from the Dockerfile, and
    // recreates the `ui` service via the /opt/authsec compose stack.
    // No registry, no Azure/AKS — the VM builds from source every time.
    environment {
        SERVICE_NAME  = 'authsec-ui'
        STACK_DIR     = '/opt/authsec'
        SRC_DIR       = '/opt/authsec/src/Authsec-ui'
        IMAGE         = 'authsec-ui:latest'
        DEPLOY_BRANCH = 'authsec-staging'
        HEALTH_URL    = 'https://app.authsec.ai/'
    }
    ...
    stages {
        ...
        stage('Health check') {
            steps {
                sh "curl -fsS --retry 12 --retry-delay 6 --retry-all-errors -o /dev/null ${HEALTH_URL}"
            }
        }
